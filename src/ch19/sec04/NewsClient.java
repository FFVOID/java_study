...
public class NewsClient {
	...
	public static void main(String[] args) {
		try {
			...
			while(true) {
				//뉴스를 받는다
				DatagramPacket recievPacket = new DatagramPacket(new byte[1024], 1024);
				datagramSocket.receive(recievPacket);
				
				//문자열로 변환
				String news = new String(recievPacket.getData(), 0, recievPacket.getLength(), "UTF-8");
